    ...
    def forward(self, x, R1):
        ...
        if hasattr(self, "inject_activation_noise") and self.inject_activation_noise:
            if hasattr(self, "activation_noise_config") and self.activation_noise_config.enabled:
                noise = self.noise_injector_fn(gate_out, self.activation_noise_config)
        ...
        if hasattr(self, '_track_layer_outliers') and self._track_layer_outliers:
            layer_idx = getattr(self, '_layer_idx', getattr(self, 'layer_idx', 'unknown'))
            layer_name = f"layer_{layer_idx}_mlp_output"
            # Reshape for tracking: (bsz, seq_len, hidden_size) -> (bsz * seq_len, hidden_size)
            flat_output = down_proj.view(-1, down_proj.size(-1))
            self._layer_outlier_tracker(flat_output, layer_name)

        return down_proj

    def forward_fixed_point(self, x, R1, prec):
        pass
...
class LlamaAttention(nn.Module):
    ...
    def __init__(self, config: LlamaConfig, layer_idx: Optional[int] = None):
        ...
        self.v_proj = NoisyLinear(
            self.hidden_size,
            self.num_key_value_heads * self.head_dim,
            rotation_config='r1wr2t', bias=config.attention_bias,
        )
    ...
    def forward(
        ...
        **kwargs,
        ...
        if hasattr(self, '_track_layer_outliers') and self._track_layer_outliers:
            layer_idx = getattr(self, '_layer_idx', getattr(self, 'layer_idx', 'unknown'))
            layer_name = f"layer_{layer_idx}_r2_value_proj"
            # Reshape for tracking: (bsz, q_len, hidden_size) -> (bsz * q_len, hidden_size)
            flat_values = value_states.view(-1, value_states.size(-1))
            self._layer_outlier_tracker(flat_values, layer_name)
        ...
        # Apply softmax noise injection using AttributeNoiseInjector pattern
        ...
        if hasattr(self, '_track_layer_outliers') and self._track_layer_outliers:
            layer_idx = getattr(self, '_layer_idx', getattr(self, 'layer_idx', 'unknown'))
            layer_name = f"layer_{layer_idx}_attn_output"
            # Reshape for tracking: (bsz, q_len, hidden_size) -> (bsz * q_len, hidden_size)
            flat_output = attn_output.view(-1, attn_output.size(-1))
            self._layer_outlier_tracker(flat_output, layer_name)
...
class LlamaDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig, layer_idx: int):
        ...
        # Initialize layer-wise outlier tracking properties
        self._track_layer_outliers = False
        self._layer_outlier_tracker = None
...
class LlamaForCausalLMCKKS(LlamaPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        # Outlier loss for rotation training - using streaming tracker
        self.noise_injector=None
        self._streaming_tracker = None
        # Storage for R2 activations cache
        self.r2_activations_cache = []
        self.use_outlier_loss = True  # Re-enable outlier loss with DDP fixes applied

        # Initialize outlier loss if enabled using streaming tracker
        if self.use_outlier_loss:
            # Use memory-efficient streaming tracker instead of factory pattern
            loss_type = getattr(config, 'outlier_loss_type', 'cvar')

            # Configure streaming tracker parameters
            tracker_config = {}
            if loss_type == 'cvar':
    ...
    def setup_noise_injection(self,
        ...
        if self.noise_injector is None:
            self.noise_injector = AttributeNoiseInjector()

        if rmsnorm_config:
            self.noise_injector.enable_rmsnorm_noise(self, rmsnorm_config)
        if softmax_config:
    ...
    def disable_all_noise(self):
        """Disable all noise injection without removing attributes."""
        if self.noise_injector:
            self.noise_injector.disable_all_noise()

    def cleanup_noise(self):
        """Clean up all noise injection attributes and reset injector."""
        if self.noise_injector:
            self.noise_injector.cleanup()
            self.noise_injector = None

    def _initialize_layer_tracking(self, config):
        """Initialize layer-wise outlier tracking infrastructure."""

        # Initialize streaming outlier tracker for memory efficiency
        self._streaming_tracker = None
        self._enable_layer_tracking = getattr(config, 'enable_layer_outlier_tracking', True)

        # Layer importance weights for loss aggregation
        self._layer_weights = self._initialize_layer_weights(config)

        # Enable layer tracking on all decoder layers
        if self._enable_layer_tracking:
            for layer in self.model.layers:
                ...
                layer.mlp._layer_outlier_tracker = self._create_layer_tracker()

    def _initialize_layer_weights(self, config):
        """Initialize uniform layer weights for ALL layers - no sampling."""
        layer_weights = {
        ...
        # Apply uniform weight to ALL layers (0 through num_layers-1)
        for i in range(num_layers):
            # R1 rotation tracking points (existing)
            layer_weights[f"layer_{i}_attn_output"] = uniform_weight
            layer_weights[f"layer_{i}_mlp_output"] = uniform_weight

            # R2 rotation tracking points (only value_proj - output_proj R2 canceled out)
            layer_weights[f"layer_{i}_r2_value_proj"] = uniform_weight

        return layer_weights

    def _get_layer_weight(self, layer_name):
        """Get the uniform weight for all layers."""
        return self._layer_weights.get(layer_name, 1.0)  # Default to uniform weight for unexpected layers

    def _create_layer_tracker(self):
        """Create a layer tracker function using streaming computation."""
        def layer_tracker(activations, layer_name):
            if self._streaming_tracker is not None:
                try:
                    # Use streaming tracker for immediate processing
                    self._streaming_tracker.track_layer(activations, layer_name)
                except Exception as e:
                    # Graceful degradation - don't break training if layer tracking fails
                    logger.warning(f"Layer outlier tracking failed for {layer_name}: {type(e).__name__}: {str(e)}")
                    # For memory errors, provide additional context
                    if "memory" in str(e).lower() or "cuda" in str(e).lower():
                        logger.warning(f"Memory issue detected in outlier tracking - consider using streaming mode")

        return layer_tracker
    ...
    def forward(
        ...
        if labels is not None:
            if self.use_outlier_loss and self._streaming_tracker is not None:
                # Use memory-efficient streaming outlier loss computation
                # All 97 layer tracking points have already been processed during forward pass

                # Track final hidden states after R1 rotation
                if hasattr(self, 'R1') and self.R1 is not None:
                    self._streaming_tracker.track_layer(hidden_states, "final_hidden_states")

                # Get aggregated loss from streaming tracker (includes all 97 tracking points)
                outlier_loss = self._streaming_tracker.get_loss()

                # Get minimal statistics for logging with batch_max compatibility
                outlier_stats = self._streaming_tracker.get_stats()
                outlier_stats['total_layers_tracked'] = 97  # All tracking points

                # Map global_max to batch_max for logging compatibility
                if 'global_max' in outlier_stats:
                    outlier_stats['batch_max'] = outlier_stats['global_max']

                # Note: R2 attention-head tracking can be added later
                # Current focus: comprehensive layer-wise R1 tracking across model depth

                # Adaptive loss scaling based on quantization precision and loss type
                loss_type = getattr(self.config, 'outlier_loss_type', 'cvar')
                ...
                if loss_type == 'cvar':
                    ...
                    scale_factor = float(os.getenv('HINGED_LOSS_SCALE', str(default_scale)))
                else:
                    scale_factor = default_scale

                # Apply scaling and log unscaled loss for monitoring
                unscaled_loss = outlier_loss.item() if hasattr(outlier_loss, 'item') else float(outlier_loss)
                loss = outlier_loss * scale_factor

                # Log outlier loss statistics every 10 steps for monitoring
                if hasattr(self, '_step_count'):
                    self._step_count += 1
                else:
                    self._step_count = 1

                # Minimal outlier logging (every 50 steps only)
                if self._step_count % 50 == 0 and outlier_stats and 'batch_max' in outlier_stats:
                    log_msg = (f"Outlier Loss (step {self._step_count}): "
                             f"loss={loss.item():.4f}, "
                             f"batch_max={outlier_stats['batch_max']:.2f}")
                    logger.info(log_msg)
            else:
