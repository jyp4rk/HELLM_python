        ...
        if args.save_qmodel_path:
            ...
            if args.export_to_et:
                save_dict = write_model_llama(
                    ...
                    0
                ]  # Export num_shards == 1 for executorch
...
def rotate_model(args, model, model_args=None):
    ...
    for layer in model.model.layers:
        rotation_utils.add_qk_rotation_wrapper_after_function_call_in_forward_v2(
            layer.self_attn,
            rope_function_name,
            config=model.config,
        )
...
def hadamard_input_projections_model(args, model, model_args=None):
    """Apply Hadamard transform to q_proj, k_proj, gate_proj, and up_proj weights with inverse transform on outputs."""
    transformers.set_seed(args.seed)
    model.eval()

    # Apply Hadamard transform to input projections
    rotation_utils.apply_hadamard_transform_to_input_projections(model)
