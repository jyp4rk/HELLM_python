...
def get_act_stat(
    model, dataloader, accumulate_type="max", prefixed_tokens=None, online_had=False
):
    model.eval()
    ...
    def stat_tensor(name, tensor, type):
        ...
        if key_name in act_stat:
            act_stat[key_name] = (
                ema_factor * act_stat[key_name] + (1 - ema_factor) * comming_max
            )
        else:
...
def inject_noise_model(model, noise_config):
    """
    replace nn.Linear and norm layer to correspond quantization counterparts
    """
    for name, module in model.named_modules():
        from eval_utils.modeling_llama import LlamaAttention, LlamaMLP, LlamaRMSNorm

        # if isinstance(module,LlamaRMSNorm) or isinstance(module,LlamaMLP) or isinstance(module,LlamaAttention):
        if isinstance(module, (LlamaRMSNorm, LlamaMLP)):
        # if isinstance(module, (LlamaRMSNorm, LlamaAttention)):
            module.setup_noise_injection(noise_config)
...
def init_weight_quantizer(args, model, minmax_init=True):
    for name, module in model.named_modules():
        if isinstance(module, int_linear_fake.QuantLinear):
            ...
            module.weight_quantizer = UniformAffineQuantizer(
                wbits,
                module.weight.shape,
                w_asym,
                w_group_size,
            ...
            print(
                f"weight quantization: set {name} as w{wbits}g{w_group_size} {sym_stat} quantization"
            )
...
def init_input_quantizer(args, model, activation_stat=None, minmax_init=True):
    for name, module in model.named_modules():
        ...
        if isinstance(module, int_linear_fake.QuantLinear):
            ...
            layer_name = name.split(".")[-1]
            if (
                layer_name in ["q_proj", "k_proj", "v_proj", "up_proj", "gate_proj"]
                or "lm_head" in name
            ):
                continue
            ...
            module.input_quantizer = UniformAffineQuantizer(
                input_bits,
                quantized_shape,
                input_asym,
                input_group_size,
            ...
            print(
                f"input activation quantization: set {name} as {input_bits}-bit {input_group_size} groupsize {input_mode} {sym_stat} quantization"
            )
            ...
            module.output_quantizer = UniformAffineQuantizer(
                output_bits,
                quantized_shape,
                output_asym,
                output_group_size,
            ...
            print(
                f"output activation quantization: set {name} as {output_bits}-bit {output_group_size} groupsize {output_mode} {sym_stat} quantization"
            )
...
def init_v_quantizer(args, model, activation_stat=None, minmax_init=True):
    ...
    for name, module in model.named_modules():
        if isinstance(module, int_linear_fake.QuantLinear) and "v_proj" in name:
            ...
            sym_stat = "asymmetric" if output_asym else "symmetric"
            print(
                f"v-cache quantization: set {name} as {output_bits}-bit {output_group_size} groupsize {output_mode} {sym_stat} quantization"
            )
    ...
    if args.k_pre_rope:
        for name, module in model.named_modules():
            if isinstance(module, int_linear_fake.QuantLinear) and "k_proj" in name:
                ...
                sym_stat = "asymmetric" if output_asym else "symmetric"
                print(
                    f"k-cache quantization: set {name} as {output_bits}-bit {output_group_size} groupsize {output_mode} {sym_stat} quantization"
                )
    else:
        for name, module in model.named_modules():
            if isinstance(module, rotation_utils.QKRotationWrapper):
                ...
                output_mode = args.kv_mode
                output_stat = (
                    activation_stat[f"{name}.output_K"]
                    if activation_stat is not None
                    else None
                )
                ...
                module.k_quantizer = UniformAffineQuantizer(
                    output_bits,
                    quantized_shape,
                    output_asym,
                    output_group_size,
...
def weight_layer_mse_init(module, input_feat, n_grid=20, max_shrink=0.5):
    ...
    for i_b in range(w.shape[0] // oc_batch_size):
        ...
        best_scale = ori_scale[
            i_b
            * quantizer.inc_groups
            * oc_batch_size : (i_b + 1)
            * quantizer.inc_groups
            * oc_batch_size
        ].clone()
        if quantizer.zero_point is not None:
            ...
            zero_point = None
        org_out = torch.einsum(
            "ngc, gcm -> ngm", input_feat.to(w_sub.dtype), w_sub.permute(1, 2, 0)
        )
        ...
        for i_s in range(int(max_shrink * n_grid)):
            ...
            cur_out = torch.einsum(
                "ngc, gcm -> ngm", input_feat.to(w_sub.dtype), q_w.permute(1, 2, 0)
            )
...
def k_cache_mse_init(module, kv_cache, softmax=False, n_grid=20, max_shrink=0.5):
    ...
    for head_index in range(num_heads):
        ...
        Q_sub = Q[
            :,
            head_index * num_key_value_groups : (head_index + 1) * num_key_value_groups,
        ]
        ...
        for i_s in range(int(max_shrink * n_grid)):
            ...
            q_K = (
                quantizer.custom_quant(q_K, cur_scale, zero_point)
                .reshape((bsz, seq_len, 1, head_dim))
                .transpose(1, 2)
                .to(Q)
            )
            cur_out = torch.matmul(
                Q_sub, repeat_kv(q_K, num_key_value_groups).transpose(2, 3)
            ) / math.sqrt(head_dim)
            if softmax:
                cur_out = nn.functional.softmax(cur_out, dim=-1, dtype=torch.float32)
            err = (cur_out - org_out).pow(2)
            err = (
                err.reshape((bsz, 1, num_key_value_groups, seq_len, seq_len))
                .mean(dim=(0, 2, 3, 4))
                .view(min_errs.shape)
            )
            del q_K
...
def tensor_mse_init_static(quantizer, data, n_grid=20, max_shrink=0.5):
    ...
    for i_s in range(int(max_shrink * n_grid)):
        ...
        err = (
            (data - q_data)
            .pow(2)
            .reshape((bs, seq_len, -1, group_size))
            .mean(dim=(0, 1, 3))
            .view(min_errs.shape)
        )
        del q_data
...
def block_mse_init_static(
    ...
    fine_search_factors = torch.linspace(
        best_clip_factor - 0.05, best_clip_factor + 0.05, 10
    ).to(dev)
    ...
    for clip_factor in fine_search_factors:
        ...
        with torch.cuda.amp.autocast():
            quant_out = qblock(
                data_inputs, position_ids=position_ids, past_key_value=past_key_value
            )[0]
        cur_loss = loss_func(quant_out, data_gts)
        if cur_loss <= best_loss:
            best_loss = cur_loss
...
def mse_init(
        ...
        if isinstance(layer, (int_linear_fake.QuantLinear, QKRotationWrapper)):
            hooks.append(
                layer.register_forward_hook(
                    get_activation_hook(
                        name, input_activation_dict, output_activation_dict
                    )
                )
            )
    ...
    for i in range(len(data_inputs)):
        with torch.cuda.amp.autocast():
            ...
            data_gts[i : i + 1] = qblock(
                data_inputs[i : i + 1],
                attention_mask=attention_mask,
                position_ids=position_ids,
                past_key_value=get_kv_cache(prefixed_key_values),
            )[0]
        ...
        if isinstance(
            ...
            if (
                ...
                if module.input_quantizer.mode == "static":
                    ...
                    logger.info(
                        f"[{name}_input_quantizer] clipping factor: ({best_clip_factor}); best_loss:{best_loss} "
                    )
                elif (
                    module.input_quantizer.mode == "dynamic"
                    and module.input_quantizer.activation_clipping
                ):
                    best_clip_factor, best_loss = block_mse_init_dynamic(
                        module.input_quantizer,
                        qblock,
                        prefixed_key_values,
                        dev,
            ...
            if (
                hasattr(module, "output_quantizer")
                and module.output_quantizer.n_bits < 16
            ):
                ...
                if module.output_quantizer.mode == "static":
                    if "v_proj" in name and module.output_quantizer.inc_groups > 1:
                        best_loss = tensor_mse_init_static(
                            module.output_quantizer, output_activation_dict[name]
                        )
                        logger.info(f"[{name}_output_quantizer] best_loss:{best_loss} ")
                    else:
                        best_clip_factor, best_loss = block_mse_init_static(
                            module.output_quantizer,
                            qblock,
                            prefixed_key_values,
                            dev,
                            data_inputs,
                        ...
                        logger.info(
                            f"[{name}_output_quantizer] clipping factor: ({best_clip_factor}); best_loss:{best_loss} "
                        )
                elif (
                    module.output_quantizer.mode == "dynamic"
                    and module.output_quantizer.activation_clipping
                ):
                    # if 'v_proj' in name and module.output_quantizer.inc_groups > 1:
                    #     pass
                    # else:
            ...
            # init k quantizer
            if (
                hasattr(module, "k_quantizer")
                and module.k_quantizer.mode == "static"
                and module.k_quantizer.n_bits < 16
            ):
                module.k_quantizer.activate()
                if module.k_quantizer.inc_groups > 1:
                    best_loss = k_cache_mse_init(
                        module, output_activation_dict[name], softmax=True
                    )
                    ...
                    logger.info(
                        f"[{name}_k_quantizer] clipping factor: ({best_clip_factor:.2f}); best_loss:{best_loss} "
                    )
            ...
            if (
                ...
                and module.weight_quantizer.n_bits < 16
            ):
                ...
                if "q_proj" in name or "k_proj" in name:
                    if args.skip_qk_weight_init:
                        ...
                        logger.info(
                            f"[{name}_weight_quantizer] clipping factor: ({best_clip_factor:.2f}); best_loss:{best_loss} "
                        )
                    else:
                        best_loss = weight_layer_mse_init(
                            module, input_activation_dict[name]
                        )
...
def set_quant_parameters(model, requires_grad):
    ...
    for n, m in model.named_parameters():
        ...
        if (
            n.find("scale") > -1
            or n.find("zero_point") > -1
            or n.find("bound_factor") > -1
        ) and (not (n.find("in_scale") > -1 or n.find("out_scale") > -1)):
            m.requires_grad = requires_grad
    return iter(params)
