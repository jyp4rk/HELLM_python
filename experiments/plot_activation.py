...
from src.utils.plot_utils import (
    ...
    plot_layer_outlier_token_num,
    plot_outlier_token_position,
    plot_outlier_token,
    plot_combined_layer_ax_input,
...
def parse_args():
    ...
    parser.add_argument(
        "--model_path",
        type=str,
        default="/cpfs01/user/chenmengzhao/llama_quantization/llama2-hf/Llama-2-7b",
        help="model path",
    )
    parser.add_argument(
        "--model_name", type=str, default="llama-2-7b", help="model name"
    )
    parser.add_argument(
        "--save_dir", type=str, default="./figures/", help="where to save the images"
    )
    parser.add_argument(
        "--dataset",
        type=str,
    ...
    parser.add_argument(
        "--max_memory", type=str, default="55GiB", help="The maximum memory of each GPU"
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="plot layer-wise outlier token number",
    )
    parser.add_argument(
        "--plot_outlier_token_position",
        action="store_true",
        help="count the token index of outlier tokens",
    )
    parser.add_argument(
        "--plot_outlier_token",
        action="store_true",
        help="count the token content of outlier tokens",
    )
    parser.add_argument(
        "--plot_outlier_token_number",
        action="store_true",
        help="count the outlier tokens number of a model",
    )
    parser.add_argument(
        "--plot_layer_input_3d",
        action="store_true",
        help="plot the 3D image of layer inputs",
    )
    parser.add_argument(
        "--plot_block_output_3d",
        action="store_true",
        help="plot the 3D image of block outputs",
    )
    parser.add_argument(
        "--disable_legend", action="store_true", help="Weather to disable the legend"
    )
    parser.add_argument(
        "--keep_prefixed_token",
        action="store_true",
        help="Weather to plot the prefixed tokens",
    )
...
if (
    args.plot_linear_input
    or args.plot_layer_input_3d
    or args.plot_layer_wise_outlier_token_number
    ...
    if isinstance(layer, class_tuple):
        layer.register_forward_hook(
            get_activation_hook(
                name,
                prefixed_length,
                args.down_online_had,
                down_had_K,
                down_K,
                args.keep_prefixed_token,
            )
        )
...
if args.plot_linear_input:
    # Back to original linear layers
    ...
    plot_combined_layer_ax_input(
        stats, args.model_name, args.save_dir, layer_names, not args.disable_legend
    )
...
if args.plot_linear_output:
    layer_names = ['q_proj', 'k_proj', 'v_proj','down_proj','up_proj','o_proj'] # plot Q/K/V, Q/K are pre rope
    # layer_names = [
...
if args.plot_layer_wise_outlier_token_number:
    stats = stat_layer_wise_outlier_token_number(
        dataloader, output_activation, model, outlier_object=args.outlier_object
    )
...
if args.plot_outlier_token_position:
    stats = stat_outlier_token_position(
        dataloader,
        output_activation,
        model,
        prefixed_tokens,
        outlier_threshold=args.outlier_threshold,
        outlier_object=args.outlier_object,
    )
...
if args.plot_outlier_token_number:
    stats = stat_outlier_token_number(
        dataloader,
        output_activation,
        model,
        outlier_threshold=args.outlier_threshold,
